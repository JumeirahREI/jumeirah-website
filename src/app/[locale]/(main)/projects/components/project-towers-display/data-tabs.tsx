...
export default function DataTabs({
  ...
  return (
    <div className="flex items-center gap-0 px-[var(--padding-x)] py-0.5 lg:gap-6">
      {tabs.map((tab, index) => {
...
function DataTabButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className="group border-gradient-t border-gradient-to-[#14141400] data-[selected=true]:border-gradient-from-[#7A7A7A99] data-[selected=true]:text-foreground border-gradient-from-[#7A7A7A00] active:text-foreground/50 relative z-30 flex flex-1 cursor-pointer items-center justify-center gap-1 rounded-2xl bg-linear-to-t from-[#1A1A1A]/0 to-[#1A1A1A]/0 px-2 py-3 text-xs text-nowrap text-[#A7AAAD] transition-colors before:transition-colors hover:bg-neutral-600/30 active:bg-neutral-600/50 data-[selected=true]:from-[#1A1A1A] md:px-3 md:text-base lg:flex-0 lg:gap-3 lg:text-lg 2xl:text-xl"
    >
      <Icon className="lg:size-8 2xl:size-10" />
      {children}
    </button>
  );
}
